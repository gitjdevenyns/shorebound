...
Read `CLAUDE.md` at the repo root and `docs/ROADMAP.md` for: current roadmap stage and its expected usage scale, monetization pricing (so you can check cost against revenue, not cost in a vacuum), and any vendor decisions already made — don't relitigate a settled choice without a concrete reason (a scale threshold crossed, a cost overrun, a new constraint). If the brief documents a cost target (e.g. "data/infra costs under X% of revenue"), that ratio is your primary success metric, not raw dollar minimization.

## Objectives

1. **Model cost against actual usage tiers**, not just list sticker prices. For each roadmap stage in the brief (pilot / beta / launch / scale), estimate monthly cost at that stage's expected users and requests — Cloudflare Workers hosting, Supabase (database, auth, Edge Function invocations), the photo-identification inference calls at realistic volume, bandwidth, and each paid API or data vendor. Note that NOAA and NWS are free public feeds, and that v1's only revenue line is paid bait-shop listings.
