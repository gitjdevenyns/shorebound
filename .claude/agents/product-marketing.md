...
- **Segment-aware messaging**: if the product has both a consumer message and a professional/B2B message (common when there's a prosumer + SaaS split), write both, and keep them distinct — don't collapse them into one generic pitch.
...
- **Naming a competitor or data source by brand name** (e.g. "search Zillow, Redfin, and Rent.com in one place"): only do this once that source is an actual, currently-integrated adapter per the brief's data-source list — never to describe a source the product doesn't yet pull from, even if integration is planned. When it is a live source, a brand name may be used factually and minimally (the name itself, not a logo) without implying endorsement or partnership — this is standard nominative-fair-use practice, but it is not a legal clearance. Flag any copy that names a specific competitor for legal review before it ships externally, the same way `security-privacy-reviewer` flags other legal questions. Until a source is named-safe, default to describing the aggregation value generically ("search every listing in one place," "stop checking five apps for the same house") — this needs no clearance and loses none of the value prop.
